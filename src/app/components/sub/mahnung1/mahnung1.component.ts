...
@Component({
  selector: 'app-mahnung1',
  templateUrl: './mahnung1.component.html',
  styleUrls: ['./mahnung1.component.scss']
})
export class Mahnung1Component {
  ...
  createPdf(task: Task) {
    ...
    doc.text("1. Mahnung: #2023-" + String(task.id) + "-2", 10, 90);

    doc.setFontSize(12);
    doc.text("Sehr geehrter Herr " + this.getCustomer(task.unternehmen) + ",", 10, 110);
    doc.text("ich möchte Ihnen gegenüber eine erste Mahnung wegen der noch ausstehenden Zahlung aussprechen.", 10, 130);
