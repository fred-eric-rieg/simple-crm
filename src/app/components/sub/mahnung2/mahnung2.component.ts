...
@Component({
  selector: 'app-mahnung2',
  templateUrl: './mahnung2.component.html',
  styleUrls: ['./mahnung2.component.scss']
})
export class Mahnung2Component {
  @Input() task: Task = {
    title: '',
    id: 0,
    fid: '',
  ...
  createPdf(task: Task) {
    ...
    doc.text("2. Mahnung: #2023-" + String(task.id) + "-3", 10, 90);

    doc.setFontSize(12);
    doc.text("Sehr geehrter Herr " + this.getCustomer(task.unternehmen) + ",", 10, 110);
    doc.text("ich möchte Ihnen hiermit eine zweite und letzte Mahnung zukommen lassen.", 10, 120);
    doc.text("Bitte überweisen Sie den offenen Betrag i.H.v. "+ (Math.round((Number(this.getTotal(task.posten)) * 1.19) * 100) / 100).toFixed(2) + " € auf das unten angegebene Konto.", 10, 130);
    doc.text("Bei Fragen stehe ich Ihnen gerne zur Verfügung.", 10, 140);

    doc.setFont("Helvetica Neue", "bold");
    doc.text("Mit freundlichen Grüßen", 10, 160);
    doc.text("Leistungserbringer GmbH", 10, 170);
